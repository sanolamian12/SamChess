/**
 * 고유기술 발동 연출 (기획 pptx 24쪽 · 2026-08-13 확장)
 *
 ...
 * | 단 | 무엇 | 길이 | 원본 |
 * |---|---|---|---|
 * | 1 | **시전자 얼굴**이 판 전체에 번쩍 떴다 부드럽게 사라진다 | 1초 | `public/battle/{장수id}.jpg` |
 * | 2 | 기술 배너 + 이름 | 2초 | `public/skills/{기술id}.jpg` |
 * | 3 | 일회성 시각 효과 (있는 기술만) | 1초 | `public/vfx/{A..G}.png` |
 *
 * 1단이 붙은 이유는 **누가 쐈는지가 안 보였기 때문**이다. 배너는 기술 그림이라
 * 이름을 읽기 전에는 시전자를 알 수 없었다.
 *
...
/** 1단 — 시전자 얼굴이 번쩍 떴다 사라지기까지 (기획자 지정 «1초») */
const FACE_MS = 1000;
/** 2단 — 배너를 띄워 두는 시간 (기획자 지정 «한 2초») */
const HOLD_MS = 2000;
