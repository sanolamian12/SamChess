 ...
 * 1초·2초에서 늘렸다 — `BattleScene.playBurstFor()`가 대사를 트는 시각(1단이
 * 시작하는 바로 그 프레임, `play()`를 부르는 자리와 같은 줄이다)과 맞춰서
 * 6초(2+4) 동안 판이 멈춰 있는다. **한때 이 대사를 `poses.ts`의 `SoundCue`
 * 시각표로 재생했는데(2026-08-31 이전), 배너가 도는 동안은 그 시각표 자체가
 * `fx.active` 때문에 멈춰 있어 대사가 배너가 다 끝난 뒤에야 늦게 들렸다** — 지금은
 * 배너를 띄우는 바로 그 호출과 나란히 직접 튼다.
 *
