...
export const POWER_MODELS: Readonly<Record<BattleMode, PowerModel>> = {
  ...
  '5v5': {
    weights: {
      ...
      // skillB는 |z|=0.7 — 표본이 「모른다」고 답했다. 0으로 둔다
...
// ═══════════════════════════════════════════════════════════════
// 4. 실측 기록 (2026-09-07 재적합) — 이 상수가 어디서 왔는가
// ═══════════════════════════════════════════════════════════════
//
//   npm run power -- 20000 --calib 5000        (모드당 2만 판, 총 522초)
//
// **왜 다시 쟀나** — 고유기술 **시전 지연**을 넣었다(2026-09-07, GDD §3.6). 13종
// (보유 **36명**)이 시전 턴을 쓰게 되고 그중 26명이 A/B급이라, 이 표의 원재료인
// `autoBattle()` 승패가 직접 흔들린다 — **의도된 재측정**이다.
// 부저추신 −1 → −4도 같은 판에 들어갔다.
//
// ── 무엇이 실제로 움직였나 (z와 함께 본다 — 계수만 보면 속는다) ──────────────
//
//   **3v3 `skillA` 15.785 → 13.826 (z=4.7 ★)** — 이번 변경의 유일하게 단단한 신호다.
//   A급 40명 중 **16명**(신기묘산 6 · 일당백 10)이 지연 대상이 되어 A급 평균 가치가
//   내려갔다. 방향·크기 모두 설명된다.
//
//   **`skillS`는 양쪽 다 거의 안 움직였다**(34.315→34.082 z=9.8 · 16.379→16.536 z=6.7).
//   강한 항이 안 움직인 것이 **설계가 의도대로 작동한다는 증거**다 — 지연이 지속시간을
//   깎지 않으므로 버프 상태로 하는 행동 횟수가 그대로다.
//
//   **`mp` 0.997→0.604 · 0.837→0.546(35~39%)은 잡음이다** — z가 2.1·2.7로 `Z_KEEP`을
//   겨우 넘긴 약한 항이고, MP 기전은 한 줄도 안 건드렸다. 5v5 `skillA`(z=2.3)도 방향은
//   같지만 근거로 삼을 만큼 단단하지 않다. **계수만 보고 이야기를 지어내면 안 된다.**
//
// ⚠ **5v5 보정이 나빠졌다 — 알고 남긴 것이다.** 아래 표에서 5v5가 −2.4 → −7.8%p로
//   벌어졌다(3v3은 −5.4 → −5.2로 그대로다). 원인은 짐작이 간다: **`skillA`가 더는
//   균질한 묶음이 아니다.** A급 4종 중 2종(16명)만 지연되고 24명은 아닌데 모형은
//   「A급 보유」를 계수 **하나**로 설명한다. 5v5 `skillA`의 z가 2.3으로 주저앉은 것도
//   같은 뿌리다. 매칭 구간(45~55%)에서는 −2.7%p라 체감 수준은 아니어서 그대로 간다.
...
//
//   3v3   예측 승률 11개 구간 전부 실제와 −5.2 ~ +0.7%p   (이전 −5.4 ~ +1.5)
//   5v5   〃                            −7.8 ~ +2.1%p   (이전 −2.4 ~ +2.1 — ⚠ 위 참조)
//
// ── 「전투력이 높은 편성이 이기는가」 (3v3, Δ등급점수 × Δ평균레벨) ─────────────
//
//              Δ레벨 ≤−3   −3~−1   −1~1    1~3    ≥3
//   Δ등급 ≤−12      0.0%    4.4%   10.4%  18.8%  55.0%
//   Δ등급  −2~2    16.1%   26.7%   43.7%  66.6%  82.5%
//   Δ등급  ≥12     46.8%   70.1%   80.8%  92.3%  99.0%
//
//   두 방향 모두 어긋난 칸 없이 단조다. P1 승 50.3%(3v3)·50.3%(5v5) — 선공 이점 없음.
//
...
//   (2026-09-07 재적합에서도 이 관계는 그대로다 — 3v3 0.471 / 1.473 · 5v5 0.284 / 0.776)
//
...
// ── 분포 (균형 성장 기준) ────────────────────────────────────────────────
//
//   3v3 부대   Lv1 D×3 = 213   ~   Lv9 S×3 = 1322
//   5v5 부대   Lv1 D×5 = 243   ~   Lv9 S×5 = 1277
//   유닛 1인   20(헌제 Lv1) ~ 692(3v3) · 386(5v5)
//
...
//
//   3v3 계수가 5v5보다 대체로 **1.9~2.8배** 크다(겹치는 건 `skillB`·`mp`뿐) —
//   한 명이 셋 중 하나일 때가 다섯 중 하나일 때보다 판을 더 크게 좌우한다.
//
//   **그래서 두 모드의 값 범위가 비슷해졌다** — 계수가 작아진 만큼 인원이 늘어
//   상쇄된다(3v3 213~1322, 5v5 243~1277). 나란히 놓으면 비교하고 싶어지는 숫자인데
//   **비교하면 안 된다.** `battlePower()`가 인원 불일치에 예외를 던지는 이유다.
//
// ── 기물을 뺀 대가 (기획자 확정, 위 주석) ────────────────────────────────
//
//   3v3   Rock +21.0 · Knight +25.4 · Queen +30.1 · Bishop −4.2 (Pawn 기준)
//         → 전투력이 같아도 기물 조합만으로 승률이 최대 8.5%p 갈린다
//   5v5   Rock +10.2 · Bishop +6.3 · Knight +10.4 · Queen +11.9 → 최대 3.0%p
//
