...
export interface PlayerProfile {
   ...
   */
  /**
   * **시장 아이템 보유 수량** — `{아이템 id: 개수}` (2026-09-23, GDD §6.5).
   *
   * 대장간처럼 자루마다 키를 두지 않는다 — 같은 것이 여러 개라 **수량**으로 센다.
   * 0이 되면 키째로 없앤다(0을 남기면 「가졌는데 없다」가 목록에 뜬다).
   *
   * **서버 소유다**(`SERVER_OWNED_FIELDS`) — 금화로 사는 것이라
   * `POST /market/item`만 늘린다.
   */
  marketOwned?: Partial<Record<string, number>>;
  /**
   * **오늘 산 수량** — 하루 매물(`시장 Lv − 해금 Lv + 1`)을 재는 값이다.
   *
   * 날짜(KST)를 함께 들고 다니며 **날이 바뀌면 「없는 것」이 된다** — 지우지
   * 않는다. 지우려면 누군가 그 순간 계정을 만져야 하는데, 아무도 안 들르는
   ...
   * 칸**이라 여기 든 장수는 지급받은 병기가 있어도 아이템을 든다 — 가르는
   * 자리는 `heldFor()` 하나다. **지급은 안 지운다**: 아이템이 소모되면 병기가
   * 그대로 돌아와야 한다.
   *
   * **클라이언트 소유다** — 총량을 안 바꾼다(대장간 지급·농지 파수꾼과 같은 결).
   */
  marketCarry?: Partial<Record<OfficerId, string>>;
  /**
   * **판이 도는 동안 빠져 있는 것** — `{장수, 아이템}`의 목록 (2026-09-23).
   *
   * 소모는 판이 **열리는 순간** 일어나는데(참가비와 같은 자리) 환불은 끝난 뒤라,
   * 그 사이에 「무엇이 나갔나」를 들고 있을 자리가 필요하다. `marketCarry`는
   * 소모할 때 비워지고, **클라이언트에게 물으면 아이템을 찍어 낼 수 있다**
   * (환불은 보유를 늘린다).
   *
 ...
 *
 * **부대는 장수를 「가리킬」 뿐이다 — 레벨을 들지 않는다** (2026-09-16 기획자 확정).
 * 한 장수가 여러 부대에 소속될 수 있고, 어느 부대에서든 **지금 보유 레벨 그대로** 선다.
 * 궁궐에서 레벨업·재설계를 하면 그 장수가 속한 부대 전부에 곧바로 반영된다.
 *
 * 예전(2026-08-18 ~ 09-16)에는 `level?`이 있어 부대마다 레벨을 낮출 수 있었다
 * (전투력을 내려 약한 상대와 붙기 위한 「하향 눈금」). **레벨업을 해도 부대에는
 * 옛 눈금이 남아** 「레벨업이 반영 안 된다」로 읽히고 부대마다 손으로 올려야 해서
 * 걷어 냈다. 옛 계정에 남은 `level`은 `migrateProfile()`이 조용히 버린다.
 */
...
export interface Squad {
  ...
  name: string;
  mode: BattleMode;
  /** 구성 — 기물과 장수의 짝뿐이다. 레벨은 언제나 장수 쪽(`roster`)에서 읽는다 */
  picks: RosterPick[];
