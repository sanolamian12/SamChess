...
그다음 HANDOFF §7의 「7. 오프라인 풀 사이클」에서 남은 **배치·정찰 화면**부터 진행한다.
...
npm run smoke:meta →  통과 — 계정·편성·전투 진입·군량·레벨업·저장 (dev 서버 필요)
```

테스트 130건의 내역 — 데이터 정합성 12 · 스케줄러/행동 35 · 책략 23 · 고유기술 19 + S급 24 ·
**메타(계정·편성·보상) 17**.
...
| 순수 로직 | `npm test` (130건) | 스케줄러·판정·책략·고유기술·**계정/편성/보상** |
| 밸런스 | `npm run balance -- 5000` | 승률·결착시간 분포. **엔진 버그도 잡는다** |
| 전투 화면 | `npm run smoke:ui` | 좌표 변환 → Intent → 판정, 교착 |
| 메타 연동 | `npm run smoke:meta` | 계정→편성→전투→레벨업→저장. **규칙과 전투를 잇는 층** |
...
7. 오프라인 풀 사이클        🔶 계정·편성·전투·결과·성장 ✅ / **배치·정찰 화면 ⬜ ← 다음**
8. Colyseus 온라인 대전      ⬜ 설계안은 아래에 있다
...
**전투 규칙·화면이 전부 섰고, 계정에서 전투까지 한 바퀴가 돈다.**
남은 것은 배치·정찰 화면, 그다음이 넷코드다.
...
| C | **배치·정찰 화면 — 아직 없다** | 엔진에는 `deploy`/`ready`/`scout`가 이미 있다 |
