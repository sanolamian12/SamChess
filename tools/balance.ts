 ...
 * ## 이 도구가 아직 재지 못하는 것 ★
 *
 * **기준 AI는 책략을 쓰지 않는다** (`rules/ai.ts`에 `castTactic`이 없다). 그래서
 * `--level`을 올려 책략을 쥐여 줘도 **지력은 여전히 승률에 반영되지 않는다** —
 * 지력이 쓰이는 곳은 환술 판정뿐이기 때문이다.
 * B급이 지력형(지력 64 · 무력 58)이라 이 한계가 특히 B급 승률을 낮게 잡는다.
 * 책략까지 재려면 AI에 시전 판단을 먼저 붙여야 한다.
 */
...
if (level > 1) {
  console.log(`\n※ 책략 ${level - 1}종을 쥐여 줬지만 **기준 AI는 책략을 쓰지 않는다** (rules/ai.ts).`);
  console.log(`   그래서 지력은 여전히 승률에 반영되지 않는다 — B급(지력형)이 특히 낮게 잡힌다.`);
}
console.log('');
