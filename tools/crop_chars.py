...
**지금 이대로 돌리면 안 된다 (2026-08-07).** `assets/Chars/`는 배경을 지운 260장으로
교체되었고, 이 도구는 배경째 잘린 그림을 쓴다 — 덮어쓰면 그 작업이 날아간다.
입력인 `assets/Images/`도 로컬에 없다 — 기획자가 별도 백업으로 옮겼다(지운 게 아니다).
다시 자를 일이 생기면 백업에서 꺼내 `--src`로 주고, `--dst`로 **다른 폴더**에 뽑은 뒤
`remove_char_background.py --src` 로 배경을 지워 `Chars/`에 넣는다.
